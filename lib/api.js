...
function collectStats(){
    ...
    async.parallel({
        ...
        network: function(callback){
            apiInterfaces.rpcDaemon('getblock', {}, function(error, reply){
                ...
                if (minerTx.vout.length != 2) {
                    log('error', logSystem, 'Coinbase transaction vout size is %d and not 2', [minerTx.vout.length]);
                }

                var reward = 0;
                // There are two transactions: the dev fee and the miner
                // reward. The last is the highest and the one we display
                // on the UI.
                for (var i=0; i<minerTx.vout.length; i++) {
                  if (minerTx.vout[i].amount > reward) {
